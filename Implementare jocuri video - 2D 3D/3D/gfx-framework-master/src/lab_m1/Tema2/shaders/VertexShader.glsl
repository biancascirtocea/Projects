...
void main() {
	// Setam variabilele uniforme pentru a le folosi in shader (amplitudinea si scalarea zgomotului)
    float u_ampl = 0.3f;
    float u_noiseScale = 5.0f;

    vec3 modifiedPosition = v_position;

    // Scalam coordonatele xz ale pozitiei pentru a obtine un zgomot mai mare
    vec2 noiseInput = v_position.xz * u_noiseScale;

    // Ajustam pozitia pe y in functie de zgomot
