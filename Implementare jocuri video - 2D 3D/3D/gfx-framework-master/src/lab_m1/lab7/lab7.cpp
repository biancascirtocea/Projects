...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(.5f, .5f, .8f));
    }

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.54f, 0.15f, 0.8f));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	glUniform1f(loc_material_kd, materialKd); // diffuse

	GLint loc_material_ks = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(loc_material_ks, materialKs); // specular
