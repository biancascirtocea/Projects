...
glm::vec3 lightPosition2;    // a doua pozitie pt sursa de lumina
glm::vec3 lightDirection2;   // directia celei de a doua surse
bool spotlight2 = false;           // spotlight pt a doua lumina
/*
...
void Lab8::Init()
{
    ...
    // initializare pozitie pt sursa 2
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.85, 0.64, 0.12));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.8, 0.36, 0.36));
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.43, 0.5, 0.56));
    }
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(1, 0.84, 0.76));
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
    ...
    int is_spotlight_loc = glGetUniformLocation(shader->program, "is_spotlight");
    glUniform1i(is_spotlight_loc, spotlight);

    int cutoff_angle_loc = glGetUniformLocation(shader->program, "cutoff_angle");
    glUniform1f(cutoff_angle_loc, cutoffAngle);
    ...
    int is_spotlight_loc2 = glGetUniformLocation(shader->program, "is_spotlight2");
    glUniform1i(is_spotlight_loc2, spotlight2);

    int cutoff_angle_loc2 = glGetUniformLocation(shader->program, "cutoff_angle2");
    glUniform1f(cutoff_angle_loc2, 30.0f);
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    // stingere spotlight pt sursa 1 de lumina
    if (key == GLFW_KEY_F)
    {
        spotlight = !spotlight;
    }

    // stingere spotlight pt sursa 2 de lumina
    if (key == GLFW_KEY_G)
    {
